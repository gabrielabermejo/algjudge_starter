...
const RUNNER_IMAGE = 'algjudge-runner-java:latest';

const submissionsQueue = new Queue(queueName, { redis: redisConfig });

console.log(`[worker_java] Listening on queue "${queueName}" at ${redisConfig.host}:${redisConfig.port}`);

submissionsQueue.process('java', async (job) => {
  ...
  try {
    log('info', 'Processing submission', { challengeId, language: 'java' });
    ...
    for (const testCase of testCases) {
      ...
      await fs.writeFile(`${tempDir}/Main.java`, code);
