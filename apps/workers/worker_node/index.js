...
const RUNNER_IMAGE = 'algjudge-runner-node:latest';

const submissionsQueue = new Queue(queueName, { redis: redisConfig });

console.log(`[worker_node] Listening on queue "${queueName}" at ${redisConfig.host}:${redisConfig.port}`);

submissionsQueue.process('node', async (job) => {
  ...
  try {
    log('info', 'Processing submission', { challengeId, language: 'node' });
    ...
    for (const testCase of testCases) {
      ...
      await fs.writeFile(`${tempDir}/code.js`, code);
