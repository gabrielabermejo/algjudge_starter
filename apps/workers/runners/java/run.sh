...
# El código ya está en /data/Main.java (montado por el worker)

# Compilar (usar /data en lugar de /tmp porque /tmp es read-only)
javac /data/Main.java 2> /data/compile_error.txt
EXIT_CODE=$?
...
# Ejecutar código
cd /data && timeout ${timeout}s java Main < "$INPUT_FILE" > /data/output.txt 2> /data/error.txt
EXIT_CODE=$?
